...
#define MAXFILENAME 140
#define DEBUG 0
#define SELECTCOUNT ((NodeTail) - (NodeCurr))>100 ? 100: (NodeTail)-(NodeCurr)+1
...
void ReceiveResponse(void)
{
	...
	if(FD_ISSET(sockfd, &writefds))
	{
		...
		time_t rawtime;
  		time (&rawtime);
		snprintf(buffer,1024,"insert into webpage(host,port,dir,page,file,timestamp) values('%s',%d,'%s','%s','%s',from_unixtime(%d));",NodeCurr->host,NodeCurr->port,NodeCurr->dir,NodeCurr->page,NodeCurr->file,(int)rawtime);
		mycon_insert(buffer);
...
__AnalyzeDone:
	close(fd);
	node->IsHandled = 1;
	//remove(node->file); //保留网页内容供以后查询快照
}
...
void AddInitNode(char * Host, char * Page, int Port, char * Dir)
{
	...
	sprintf(filename, "/tmp/webpage/file%05d.html", FileNumber++);
